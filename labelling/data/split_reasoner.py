"""Split reasoner labels into train/val/test with issue-level stratification.

Splits on issue_number (not rows) to prevent data leakage across snapshot tiers.
Stratifies by band (medium vs high) so each split has proportional representation.

Run from labelling/: python -m data.split_reasoner
"""
...
# ── Config ──────────────────────────────────────────────────────────────
INPUT_FILE  = Path(__file__).resolve().parent / "labeled" / "reasoner" / "reasoner_labels.jsonl"
OUTPUT_DIR  = Path(__file__).resolve().parent / "labeled" / "reasoner"
TRAIN_RATIO = 0.80
VAL_RATIO   = 0.10
TEST_RATIO  = 0.10
SEED        = 42
...
BAND_PRIORITY = {"medium": 0, "high": 1}


def get_issue_band(rows: list[dict]) -> str:
    bands = [r.get("scorer_band", "medium") for r in rows]
...
for split_name, split_records in [("Train", train_records), ("Val", val_records), ("Test", test_records)]:
    ...
    for r in split_records:
        band_counts[r.get("scorer_band", "medium")] += 1
